	...
	<?php if ( $len > 0 ) : ?>
		<?php if ( $layout == "layout1" ) : ?>
            ...
            <section class="carousel layout1-container">
				<?php for ( $i = 1; $i <= $len; $i ++ ) : ?>
                    <input class="carousel-locator" id="slide-<?php echo $i; ?>" type="radio" name="carousel-radio"
                           hidden="" <?php echo $i == 1 ? 'checked' : ''; ?>>
				<?php endfor; ?>
                <div class="carousel-container">
					<?php for ( $i = 1; $i <= $len; $i ++ ) : ?>
                        <figure class="carousel-item">
                            <label class="item-prev btn btn-action btn-lg"
                                   for="slide-<?php echo $link_to[ $i - 1 ][0]; ?>">
                                <i class="icon icon-arrow-left"></i>
                            </label>
                            <label class="item-next btn btn-action btn-lg"
                                   for="slide-<?php echo $link_to[ $i - 1 ][1]; ?>">
                                <i class="icon icon-arrow-right"></i>
                            </label>
                            <div class="img-responsive rounded"
                                 style="background-image:url(<?php echo $carousels[ $i - 1 ]; ?>)">
                            </div>
                        </figure>
					<?php endfor; ?>
                </div>
